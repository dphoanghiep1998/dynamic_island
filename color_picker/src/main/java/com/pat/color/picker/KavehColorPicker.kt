...
class KavehColorPicker(context: Context, attributeSet: AttributeSet?) :
    ...
    var circleIndicatorRadius = dp(8)
        set(value) {
            field = value
            invalidate()
        }
    ...
    private var colorWithFullAlpha1 = Color.RED
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(
            max(finalWidth, suggestedMinimumWidth), max(finalHeight, suggestedMinimumHeight)
        )
    }
    ...
    override fun onDraw(canvas: Canvas) {
        canvas.run {
            ...
            drawCircle(circleX, circleY, circleIndicatorRadius, circlePaint.apply {
                color = strokeColor
            })

            drawCircle(circleX, circleY, circleIndicatorRadius - strokeSize, circlePaint.apply {
                color = colorWithFullAlpha1
            })
        }
    }
    ...
    override fun initializeSliderPaint() {
        ...
        darknessShader = LinearGradient(
            0f, drawingTop, 0f, heightF, Color.TRANSPARENT, Color.BLACK, Shader.TileMode.MIRROR
        )
